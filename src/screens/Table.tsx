  ...
  return (
    <div className="min-h-screen p-2 sm:p-4 lg:p-8 pb-40 sm:pb-28 lg:pb-32">
      ...
      {/* Сеть есть, а записать не выходит — сервер недоступен. Случай отдельный
          ...
          Текст — короткая памятка, а не объяснение. Через месяц человек не
          вспомнит разговор, ему нужно сразу понять: что не страшно, чего
          нельзя, и что делать дальше (замечание Андрея 09.09.2026).
          Прежнее «не закрывай вкладку» убрано: это был лишний страх, партия
          лежит в памяти планшета и переживает закрытие приложения. */}
      {syncState === 'failed' && isOnline && (
        <div className="mb-5 px-5 py-4 bg-red-500/15 border border-red-500/50 rounded-lg">
          ...
          <div className="text-base text-red-100/80 mt-2 space-y-1">
            <div>
              <b>Партия не пропадёт</b> — она записана в памяти этого планшета. Играйте дальше,
              можно даже закрыть приложение.
            </div>
            <div>
              <b>Нельзя:</b> открывать эту партию на другом устройстве и чистить данные браузера.
            </div>
            <div>
              <b>Дальше:</b> связь вернётся — уедет сама. Или в списке партий кнопкой «загрузить в
              облако».
            </div>
          </div>
        </div>
