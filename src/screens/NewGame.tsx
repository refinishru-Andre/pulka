...
// «на время» вместо «без предела»: короче — влезает в кнопку на телефоне —
// и понятнее, потому что на турнире именно так и играют (Андрей, 09.09.2026).
const poolLabel = (n: number | null) => (n === null ? 'на время' : String(n))
...
function RulesSummary({ rules }: { rules: Rules }) {
  ...
  if (rules.prikupBonus) lines.push('Сдающий получает висты за прикуп')
...
function PoolLimitPicker({
  ...
  return (
    <div>
      ...
      {poolLimit === null && (
        <div className="text-xs text-slate-500 mt-2">
          Пуля не закрывается: доиграли — нажали «Рассчитать партию».
        </div>
      )}
    </div>
  )
