...
export function calcNet(state: GameState): Record<PlayerId, number> {
  ...
  seats.forEach((p) => {
    ...
    const owed = seats.reduce((s, other) => (other === p ? s : s + (state.whists[other]?.[p] ?? 0)), 0)
...
export function calcPairwise(
  ...
  const positive = seats.filter((p) => net[p] > 0).map((p) => ({ p, amount: net[p] }))
  const negative = seats.filter((p) => net[p] < 0).map((p) => ({ p, amount: -net[p] }))
...
export function settle(state: GameState): Settlement {
  ...
  const pairwise = calcPairwise(net, seats)
  // Округляем net для отображения
  const netRounded: Record<PlayerId, number> = zeroScores()
  seats.forEach((p) => (netRounded[p] = Math.round(net[p])))
