// ЭТАЛОН. Это движок Людочки, каким он работал до появления игры вчетвером и
// конвенций — снят с ветки main без единой правки.
//
// Зачем: чтобы можно было ДОКАЗАТЬ, что новый движок считает партии на троих
// точно так же, как считала прежняя Людочка. Тест reference.test.ts прогоняет
// через оба движка тысячи случайных партий и сверяет каждую цифру.
//
// НЕ ПРАВИТЬ И НЕ ИМПОРТИРОВАТЬ В ПРИЛОЖЕНИЕ. Только для тестов.
...
export type Contract =
  // suit заполняется ТОЛЬКО для Сталинграда (6♠) — единственный случай, где масть
  // влияет на расчёт (оба вистующих обязаны вистовать). Во всех остальных играх
  // масть не участвует ни в одной формуле, поэтому её не спрашиваем и не храним.
